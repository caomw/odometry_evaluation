...
def load_data(ground_truth_file, odometry_file):
    # load float data
    ground_truth = pylab.loadtxt(ground_truth_file, delimiter=',', skiprows=1, 
            usecols=(0,5,6,7,8,9,10,11))
    odometry = pylab.loadtxt(odometry_file, delimiter=',', skiprows=1, 
            usecols=(0,5,6,7,8,9,10,11))
    # log is in nanoseconds, so we have to convert to seconds
    ground_truth[:,0] = ground_truth[:,0]/1e9
